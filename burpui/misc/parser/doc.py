...
class Doc(BUIparser):
    ...
    doc = {
        '.': __("Read additional configuration files. On Windows, the glob"
                " is unimplemented - you will need to specify an actual"
                " file."),
        'address': __("Defines the main TCP address that the server listens"
                      " on. The default is either '::' or '0.0.0.0',"
                      " dependent upon compile time options."),
        'atime': __("This allows you to control whether the client uses"
        ...
        'autoupgrade_dir': __("Path to autoupgrade directory from which"
                              ...
                              " option."),
        'ca_burp_ca': __("Path to the burp_ca script when using the ca_conf"
                         " option."),
        'ca_conf': __("Path to certificate authority configuration file. The"
        ...
        'compression': __("Choose the level of gzip compression for files"
                          ...
                          " server."),
        'cross_all_filesystems': __("Allow backups to cross all filesystem"
                                    " mountpoints."),
        'cross_filesystem': __("Allow backups to cross a particular"
                               " filesystem mountpoint."),
        'daemon': __("Whether to daemonise. The default is 1."),
        'dedup_group': __("Enables you to group clients together for file"
        ...
        'directory_tree': __("When turned on (which is the default) and the"
        ...
        'exclude_fs': __("File systems to exclude from the backup. Case"
                         ...
                         " exclude tmpfs."),
        'exclude_regex': __("Exclude paths that match the regular"
                            " expression."),
        'exclude': __("Path to exclude from the backup. You can have"
        ...
        'group': __("Run as a particular group. This can be overridden by"
                    " the client configuration files in clientconfdir on the"
                    " server."),
        'hard_quota': __("Do not back up the client if the estimated size of"
                         " all files is greater than the specified size."
                         " Example: 'hard_quota = 100Gb'. Set to 0 (the"
                         " default) to have no limit."),
        'hardlinked_archive': __("On the server, defines whether to keep"
        ...
        'include_ext': __("Extensions to include in the backup. Case"
        ...
        'include': __("Path to include in the backup. You can have multiple"
                      " include lines. Use forward slashes '/', not"
                      " backslashes '\\' as path delimiters."),
        'include_glob': __("Include paths that match the glob expression."
                           ...
                           " contain only one '*'."),
        'keep': __("Number of backups to keep. This can be overridden by the"
        ...
        'librsync': __("When set to 0, delta differencing will not take"
        ...
        'lockfile': __("Path to the lockfile that ensures that two server"
                       " processes cannot run simultaneously."),
        'manual_delete': __("If a path is given, the server will move"
        ...
        'max_file_size': __("Do not back up files that are greater than the"
                            " specified size. Example: 'max_file_size ="
                            " 10Mb'. Set to 0 (the default) to have no"
                            " limit."),
        'max_hardlinks': __("On the server, the number of times that a single"
        ...
        'notify_success_script': __("Path to the script to run when a backup"
                                    ...
                                    " the server."),
        'notify_success_warnings_only': __("Set to 1 to send success"
        ...
        'password_check': __("Allows you to turn client password checking on"
                             ...
                             " the server."),
        'path_length_warn': __("When this is on, which is the default, a"
        ...
        'protocol': __("Choose which style of backups and restores to use. 0"
        ...
        'ratelimit': __("Set the network send rate limit, in Mb/s. If this"
                        " option is not given, burp will send data as fast as"
                        " it can."),
        'read_all_blockdevs': __("Open all block devices for reading and"
                                 " back up the contents as if they were"
                                 " regular files."),
        'read_all_fifos': __("Open all fifos for reading and back up the"
                             " contents as if they were regular files."),
        'read_blockdev': __("Do not back up the given block device itself,"
                            " but open it for reading and back up the"
                            " contents as if it were a regular file."),
        'read_fifo': __("Do not back up the given fifo itself, but open it"
                        " for reading and back up the contents as if it were"
                        " a regular file."),
        'restore_client': __("A client that is permitted to list, verify,"
                             ...
                             " burp."),
        'resume_partial': __("Turn this on to enable 'resume partial' code."
        ...
        'scan_problem_raises_error': __("When enabled, this causes problems"
        ...
        'server_script_post_arg': __("A user-definable argument to the"
                                     " server post script. You can have many"
                                     " of these."),
        'server_script_post_notify': __("Turn on to send a notification"
                                        ...
                                        " set."),
        'server_script_post_run_on_fail': __("If this is set to 1,"
                                             ...
                                             " run."),
        'server_script_post': __("Path to a script to run on the server"
                                 ...
                                 " clientconfdir on the server."),
        'server_script_pre_arg': __("A user-definable argument to the server"
                                    " pre script. You can have many of"
                                    " these."),
        'server_script_pre_notify': __("Turn on to send a notification email"
                                       ...
                                       " notify_failure options to be set."),
        'server_script_pre': __("Path to a script to run on the server after"
                                ...
                                " on the server."),
        'server_script': __("You can use this to save space in your config"
                            ...
                            " client configuration files in clientconfdir on"
                            " the server."),
        'soft_quota': __("A warning will be issued when the estimated size"
                         ...
                         " warning."),
        'split_vss': __("When backing up Windows computers with burp"
        ...
        'ssl_cert_ca': __("The path to the SSL CA certificate. This file"
        ...
        'ssl_cert': __("The path to the server SSL certificate. It works for"
                       " me when the file contains the concatenation of the"
                       " certificate and private key in PEM format."),
        'ssl_ciphers': __("Allowed SSL ciphers. See openssl ciphers for"
                          " details."),
        'ssl_compression': __("Choose the level of zlib compression over"
                              ...
                              " 'zlib'."),
        'ssl_dhfile': __("Path to Diffie-Hellman parameter file. To generate"
                         " one with openssl, use a command like this: openssl"
                         " dhparam -out dhfile.pem -5 1024"),
        'ssl_key_password': __("The SSL key password."),
        'ssl_key': __("The path to the server SSL private key in PEM"
                      " format."),
        'ssl_peer_cn': __("Must match the common name in the SSL certificate"
        ...
        'strip_vss': __("When backing up Windows computers with burp"
        ...
        'timer_arg': __("A user-definable argument to the timer script."
                        "You can have many of these. The timer_arg options"
                        " can be overridden by the client configuration files"
                        " in clientconfdir on the server."),
        'timer_script': __("Path to the script to run when a client connects"
                           ...
                           " overridden by the client configuration files in"
                           " clientconfdir on the server."),
        'timestamp_format': __("This allows you to tweak the format of the"
        ...
        'user': __("Run as a particular user. This can be overridden by the"
                   " client configuration files in clientconfdir on the"
                   " server."),
        'version_warn': __("When this is on, which is the default, a warning"
                           " will be issued when the client version does not"
                           " match the server version. This option can be"
                           " overridden by the client configuration files in"
                           " clientconfdir on the server."),
        'vss_drives': __("When backing up Windows computers, this option"
        ...
        'working_dir_recovery_method': __("This option tells the server what"
